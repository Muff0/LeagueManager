...
public static class ReviewExtensions
{
    ...
    {
        return string.Format(Templates.LongMatchTitleTemplate,
            match.SeasonName,
            match.Round,
    ...
    public static ReviewDto ToReviewDto(this ReviewViewModel reviewViewModel)
    {
    ...
    {
        var blackPlayer = review.Match?.PlayerMatches?.FirstOrDefault(pm => pm.Color == PlayerColor.Black)?.Player;
        var whitePlayer = review.Match?.PlayerMatches?.FirstOrDefault(pm => pm.Color == PlayerColor.White)?.Player;
